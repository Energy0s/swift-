...
import { mt101Api, type Mt101Message } from '../services/mt101Service';
...
const Mt101ListPage: React.FC = () => {
  const navigate = useNavigate();
  const [messages, setMessages] = useState<Mt101Message[]>([]);
  ...
  }, [statusFilter, mtTypeFilter, referenceFilter, dateFrom, dateTo, refreshKey]);

  const formatDate = (d: string) => new Date(d).toLocaleDateString('pt-BR');
  const formatAmount = (msg: Mt101Message) => {
    const total = msg.transactions.reduce((s, t) => s + t.amount, 0);
    const curr = msg.transactions[0]?.currency || 'EUR';
    return `${curr} ${total.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`;
  };
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2, flexWrap: 'wrap', gap: 2 }}>
        ...
        <Button variant="contained" startIcon={<AddIcon />} onClick={() => navigate('/mt101/new')}>
          Nova mensagem
        </Button>
      </Box>
        ...
        <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', alignItems: 'center' }}>
          <FormControl size="small" sx={{ minWidth: 140 }}>
            ...
            <Select value={mtTypeFilter} onChange={(e) => setMtTypeFilter(e.target.value)} label="MT Type">
              ...
              <MenuItem value="MT101">MT101</MenuItem>
              <MenuItem value="MT102">MT102</MenuItem>
              <MenuItem value="MT102STP">MT102+ (STP)</MenuItem>
            </Select>
          </FormControl>
          <TextField
            size="small"
            ...
            {messages.length === 0 ? (
              ...
              messages.map((m) => (
                <TableRow key={m.id} hover>
                  <TableCell sx={{ fontFamily: 'monospace', fontWeight: 600 }}>{m.mtType || 'MT101'}</TableCell>
                  <TableCell sx={{ fontFamily: 'monospace' }}>{m.transactionReferenceNumber}</TableCell>
                  <TableCell>
                    <Chip
                      label={m.messageStatus}
                      size="small"
                      color={STATUS_COLORS[m.messageStatus] || 'default'}
                    />
                  </TableCell>
                  <TableCell>{formatDate(m.executionDetails?.requestedExecutionDate || m.createdAt)}</TableCell>
                  <TableCell>{m.transactions.length}</TableCell>
                  <TableCell align="right" sx={{ fontFamily: 'monospace' }}>
                    {formatAmount(m)}
                  </TableCell>
                  <TableCell sx={{ maxWidth: 150, overflow: 'hidden', textOverflow: 'ellipsis' }}>
                    {m.orderingCustomer?.orderingCustomerName || '-'}
                  </TableCell>
                  <TableCell align="right">
                    <Button size="small" startIcon={<ViewIcon />} onClick={() => navigate(`/mt101/${m.id}`)}>
                      Ver
                    </Button>
                  </TableCell>
                </TableRow>
